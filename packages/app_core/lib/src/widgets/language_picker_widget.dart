...
class LanguageWidget extends StatefulWidget {
  const LanguageWidget({Key? key, required this.locales, this.selected = const Locale('en', 'US')}) : super(key: key);

  @override
  LanguageWidgetState createState() {
    return LanguageWidgetState();
  }
...
class LanguageWidgetState extends State<LanguageWidget> {
  ...
  @override
  void initState() {
    selectedLanguage = widget.selected.languageCode;
    list = widget.locales.map((Locale value) {
      return DropdownMenuItem<String>(
        value: value.languageCode,
        child: Text(language.getNativeName(value.languageCode)),
      );
    }).toList();
    super.initState();
  }

  Widget _languageCard(Locale locale) {
    final name = language.getNativeName(locale.languageCode);
    return Card(
      child: Center(
        child: ListTile(
          ...
          onTap: () {
            ...
            final lang = Language(
              ...
              name: name,
              nativeName: language.getNativeName(locale.languageCode),
            );
            Navigator.of(context).pop(lang);
          },
          leading: CircleAvatar(
            backgroundImage: ExactAssetImage(
              'assets/flags/${locale.countryCode?.toLowerCase()}.png',
              package: 'app_core',
            ),
          ),
          title: Text(name).h3(),
          subtitle: Text(language.getDisplayLanguage(locale.languageCode)),
            ...
            children: [
              ...
              ConstrainedBox(
                ...
                child: ListView(
                  shrinkWrap: true,
                  children: [
                    for (Locale item in widget.locales) ...[
                      _languageCard(item),
                      VSpace.sm,
                    ],
                  ],
...
class LanguageLocale {
  ...
  String getDisplayLanguage(String key) {
  ...
  String getNativeName(String key) {
    if (isoLangs.containsKey(key)) {
      return isoLangs[key]!['nativeName'] ?? isoLangs[key]!['name'] ?? '';
    } else {
      return '';
    }
  }
